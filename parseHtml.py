...
#解析播放页面
def parsePlayView(videoType,movieName,url):

    # 加载子菜单内容列表下内容详情
    detailReq = requests.get(kBaseUrl + url)
    detailData = detailReq.text.encode(detailReq.encoding).decode('utf-8')
    # print(detailData)
    ...
    if indexStr.__contains__('"'):
        ...
        print(videoType + '/' + movieName + totalUrl)

        # conList.append(movieName + totalUrl + '\n')

        return movieName + totalUrl + '\n'
...
def parsePlayData(videoType,url):
    ...
    for t in tt:
        ...
        content = parsePlayView(videoType,t.text,t.a['href'])
        conList.append(content)
        # f.write(content)

    # f.close()
    writeDataToFile(videoType,conList)
        ...
        for i in range(1, len(soup2.contents) - 1):
            ...
            if soup2.contents[i].string != '\n':
                ...
                if soup2.contents[i].a['href'] == '/':
                    ...
                    # 菜单下的内容
                    content = '%s : %s%s' % (soup2.contents[i].string, kBaseUrl, soup2.contents[i].a['href'])

                    if soup2.contents[1].string == '视频区':

                        print(soup2.contents[i].string + soup2.contents[i].a['href'])
...
def writeDataToFile(fileName, data):
    ...
    path = '/Users/wang/Desktop/data/' + fileName
